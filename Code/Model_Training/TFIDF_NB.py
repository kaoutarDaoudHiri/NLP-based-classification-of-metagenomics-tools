...
X_test.to_pickle("/home/khiri/local/MetagenomicToolsClassifier/Abstracts/ML/BagOfWords/06-02-21_run/NB-X_test.pkl")
y_test.to_pickle("/home/khiri/local/MetagenomicToolsClassifier/Abstracts/ML/BagOfWords/06-02-21_run/NB-y_test.pkl")

X_train.to_pickle("/home/khiri/local/MetagenomicToolsClassifier/Abstracts/ML/BagOfWords/06-02-21_run/NB-X_train.pkl")
y_train.to_pickle("/home/khiri/local/MetagenomicToolsClassifier/Abstracts/ML/BagOfWords/06-02-21_run/NB-y_train.pkl")

# Pipeline & GridSearch Parameter setup
tvc_pipe = Pipeline([
...
# Params
tf_params = {'tvec__max_features':[100, 140, 180, 220],
...
for i in range(NUM_TRIALS):
    ...
    for train_idx, val_idx in tqdm(cv_outer.split(X_train, y_train)):
        ...
        gd_search = GridSearchCV(model, tf_params, scoring='balanced_accuracy', n_jobs=-1, cv=cv_inner)
...
tvc_pipe = Pipeline([('tvec', TfidfVectorizer(max_features=d2['Best Params']['tvec__max_features'],
...
model = tvc_pipe.fit(X_train, y_train)
with open('/home/khiri/local/MetagenomicToolsClassifier/Abstracts/ML/BagOfWords/06-02-21_run/best_nbc.pickle', 'wb') as output:
...
df_models_nbc = pd.DataFrame(d, index=[0])
df_models_nbc

with open('/home/khiri/local/MetagenomicToolsClassifier/Abstracts/ML/BagOfWords/06-02-21_run/df_models_nbc.pickle', 'wb') as output:
        pickle.dump(df_models_nbc, output)
